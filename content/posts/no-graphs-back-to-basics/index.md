...
What replaces it is the wrapper-lane at `dotfiles/SST3/scripts/sst3-code-*`, scoped as Phase A of a four-phase plan in `dotfiles#445` (the four phases compose: code wrappers, doc-tooling wrappers, cross-class drift detection, harness integration). Nine scripts, one per query type (status, update, search, callers, callees, impact, large, review, untested-py). None of them are a graph... no SQLite persistent layer, no Tree-sitter index, no embeddings. They are stateless Bash wrappers that reproduce the old query interface using `ast-grep` for structural patterns, `ripgrep` for literal search, `git` for diff and log, `find` for file enumeration, and `coverage.py` for untested-function detection per language. You invoke a script from the Bash tool inside an agent (main or subagent), it runs, it prints NDJSON (newline-delimited JSON), it exits. There is no `~/.claude.json` entry. There is no long-running daemon. Each query starts from nothing and finishes returning nothing in memory. When the next session resumes, there is zero stdio state to replay. Zero!

The contract surface is preserved by JSON-shape match. Scripts emit the same shape of output the old MCP would have, so callers in our canonical docs stay textually correct against the new lane. Same arguments, same output shape. The wiring underneath is different. The wiring above does not need to know.

The architecture has three layers, scoped across `dotfiles#445`. Layer 1 is the wrapper scripts themselves, shipped in Phase A. Layer 2 will be a thin orchestrator (`sst3-check.sh`), planned for Phase D. Layer 3 will be skill-and-hook integration into the harness, also Phase D. Layered failure isolation is the point of the design. Kill the skill (Layer 3) and the orchestrator plus the CLI scripts still run from any shell. Kill the orchestrator (Layer 2) and the scripts still run from any shell. The bottom layer is always usable for ground-truth debugging. That is what you cannot get from a daemon-MCP coupling. When the daemon is down, the daemon is down, and you find yourself debugging the transport, not the question.
