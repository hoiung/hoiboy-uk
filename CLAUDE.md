...
**Every singular indexable page gets its own 1200×630 Open Graph / Twitter share card.** Consulting + client case-study (portfolio) pages and the **legal** pages (`content/legal/*`) get a generated text card; posts use their `hero.*`. Home + taxonomy + section index pages share the branded **`content/default-card.png`**, which replaced the old `hoi-mug.jpg` face as the site-wide fallback. A deterministic guard — `scripts/check_social_cards.py`, wired into pre-commit + pre-publish + CI (with a rendered-HTML template-regression backstop) — fails the build if a singular page is a flat `.md`/`.markdown`/`.html` (cannot hold a card) or would fall back to the default card, so a future page cannot silently ship the generic card. Text cards are generated by `scripts/social-cards/gen_card.py` from `cards.tsv` (consulting) + `legal-cards.tsv` (legal), in the `hoiboy` or `agit` style. Reusable design + full detail: `scripts/social-cards/README.md` (brand tokens from `docs/research/07_DESIGN_TOKENS.md` + retro VT323 / IBM Plex Mono type + the square `hoiboy.uk` logo signature at a symmetric corner inset). Client case-study cards use the **same template, no client logo** (consistency with the service cards). Quick version:

1. Add a row to `scripts/social-cards/cards.tsv`: `<slug><TAB><title><TAB><AI-explicit tagline>` (slug = the page-bundle dir under `content/hire-hoi/ai-consultancy/`; **nested slugs work as-is**, e.g. `portfolio/cu-architects`)
2. Run `python3 scripts/social-cards/gen_card.py` (needs `rsvg-convert` + Pillow) → writes `content/hire-hoi/ai-consultancy/<slug>/share-card.png` at 1200×630
