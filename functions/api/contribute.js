...
const FROM_ADDR = "agit-form@hoiboy.uk";
...
// submitter's email address into a log line, which contradicts what
...
export async function onRequestPost(context) {
  ...
  const { log, protect, redact } = createLogger("contribute");
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      const detail = redact(await readCappedText(resp, UPSTREAM_BODY_CAP)).slice(0, 500);
