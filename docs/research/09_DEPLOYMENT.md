...
| `AGIT_MAILER` | `send_email` binding | Settings > Bindings > Add > Send email | Cloudflare-native send from `noreply@hoiboy.uk` to the verified destination `hoiboyuk@gmail.com` |
...
**Email: the zero-DNS-chore path.** The Function calls `env.AGIT_MAILER.send({to, from, subject, text, attachments})` (Cloudflare Email Service object builder). Per Cloudflare docs, sends to a **verified destination address** are free on any plan **when only Email Routing is configured**, so you do NOT need to onboard the domain to Email Sending (no `cf-bounce` SPF/DKIM/DMARC records). The only requirements: (a) the `to` is a **verified Email Routing destination address** (`hoiboyuk@gmail.com`, the address the `hello@hoiboy.uk` routing rule already forwards to; note a routing-rule address like `hello@hoiboy.uk` is NOT itself a destination address and cannot be a send target), and (b) the `from` is on a **routing domain** (`noreply@hoiboy.uk`, since `hoiboy.uk` is the routing domain; the local part is synthetic and needs no registration). Onboarding `hoiboy.uk` to Email **Sending** is only needed to email arbitrary (non-verified) recipients, which this form does not do.
...
3. Email: confirm `hoiboyuk@gmail.com` is a **verified** Email Routing destination address (Email Service > Email Routing > Destination Addresses; it already is, as the `hello@hoiboy.uk` routing rule forwards to it), then add the `send_email` binding named `AGIT_MAILER` to the Pages project (allowed sender `noreply@hoiboy.uk`, destination `hoiboyuk@gmail.com`). No Email Sending domain onboarding is required for this verified-destination path. Keep the `hello@hoiboy.uk` routing rule regardless, since it is the site's public contact address and is independent of the form. Redeploy so the binding takes effect.
