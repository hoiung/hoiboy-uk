...
2. Run `python3 scripts/social-cards/gen_card.py` (needs `rsvg-convert` + Pillow); no args regenerates all six sets → writes `share-card.png` at 1200×630 into each bundle (`content/share-card.png` for home). Single set: `gen_card.py landings` / `gen_card.py home`.
