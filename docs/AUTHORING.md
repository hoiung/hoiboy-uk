...
**Validator**: `python3 scripts/validate_frontmatter.py` (also runs in CI and pre-commit). Hard fail on missing required fields or unknown categories. Remaining optional fields are not enforced for backward compatibility. It does **not** validate YAML in general: the parser is hand-rolled, it understands the shapes real posts use, and malformed YAML is caught later by the real Hugo build (pre-publish gate 7 and the CI build step), not here. Ralph round 18 confirmed the gap in both directions: a tab-indented block scalar passes this validator and then hard-fails `hugo`, and a few exotic empty forms (`description: !!str`, an alias to an anchored empty string) still pass presence while Hugo serves the site default. Swapping the parser for PyYAML is tracked as its own scoped change (operator decision, 2026-07-20); the exact divergent shapes are listed in `parse_frontmatter`'s docstring in `scripts/validate_frontmatter.py`.
