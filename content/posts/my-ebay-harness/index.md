...
Started 10 April 2026. Latest commit 25 April 2026. Fifteen days. 160 commits. 135 of them Claude-attributed (about 84%). Ten Ralph-reviewed merges. Public on GitHub, MIT licensed, [hoiung/ebay-seller-tool](https://github.com/hoiung/ebay-seller-tool).
...
MCP makes the synthesis possible because every signal is already a tool the agent can ask for. The harness is the thing that decides which tools to ask for, in what order, with what fallback when a call fails. The agent sees seventeen tools. It does not see seventeen REST endpoints. The orchestration logic, the rules about which tool when and which refusal when, lives in the harness, not in the prompt.
