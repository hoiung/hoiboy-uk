...
def main() -> int:
    if os.environ.get("SST3_PROTECTED_TAG_OVERRIDE") == "1":
        print("[sst3-protected-tag-guard] SST3_PROTECTED_TAG_OVERRIDE=1 — bypassed.", file=sys.stderr)
        return 0
    ...
    for raw in sys.stdin:
        ...
        if len(parts) < 4:
            continue
        local_ref, local_sha, remote_ref, _remote_sha = parts[0], parts[1], parts[2], parts[3]
    ...
    print("[sst3-protected-tag-guard] BLOCKED — local-only tags in push:", file=sys.stderr)
    for tag, pat in blocked:
        print(f"  refs/tags/{tag}  (matches protected pattern '{pat}')", file=sys.stderr)
    print("", file=sys.stderr)
    print("These tags are local-only safety bookmarks (rebase backups, pre-merge", file=sys.stderr)
    print("checkpoints, WIP markers). If you genuinely need to push one (e.g. for", file=sys.stderr)
