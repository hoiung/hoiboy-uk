...
5. Per-harness build, custom-scoped per audit (the audit is what tells us what to build, so I will not pre-quote it). Typical band £12K-£30K + VAT depending on harness complexity.
...
| 5. Per-harness build | £12K-£30K typical (fixed-fee output of audit) | Custom-scoped per audit; volume discount for multi-harness commitments at engagement open |
...
| 9. Out-of-scope work (new features, additional harnesses, mid-engagement changes) | Custom-quoted via fresh per-harness audit (£750/SME-day); build is fixed-fee output of that audit | Same audit-first discipline as the original engagement; small change-orders may qualify for Tier 2 hourly pack if you have one |
