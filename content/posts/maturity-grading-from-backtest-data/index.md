...
The box was a lie. In this 4% cut, only three of the eight corners stand out (all above +2.28R with one-in-six odds of stopping out). Below them there's a real cliff, a drop of roughly 0.9R, and the rest fall off hard. Quick word on what that cliff is actually measuring, because it's an easy one to misread. It's not the gap to whichever row sits directly below B·B·E (the rows under the line aren't sorted by R), and it's not the top-to-bottom spread of the whole table either. It's the drop from the worst of the top three (B·B·E at +2.28R) down to the best of everything underneath it (A·B·E at +1.38R), with nothing averaging in between. That empty band is the cliff. The A·B ones (A·B·E, A·B·D) sit lower here because the daily has run hotter than the weekly. Not out of sync exactly, the weekly is still confirming underneath, just a notch less mature than the leaders.

### What the cliff is telling me

And that gap is the useful part, not just a quirk of the table. A clean break like that means the top three aren't the top of a smooth slope, each shape a touch worse than the last. They're a separate cluster. If the numbers faded away gently I'd be guessing where to draw my cutoff, but the empty band draws it for me. And stepping across it costs you twice over: the reward falls off by a third or more, and the stop-out rate climbs from around one-in-six up toward one-in-three and beyond, both at the same time. There's no "close enough" here. You're either in one of those shapes or you're not.

What sold me on it wasn't this one table though. I ran the same shapes across the full pile of trades, not just the filtered slice I'd actually trade, and the gap was still sitting there. Same on the weekly setups taken on their own. There's a CLIFF tab on the combo tables at the bottom of this post that lays it out for every view and every cut, and the drop keeps turning up. A line that survives that much poking isn't a fluke of one slice. It's the shape of the thing.
...
And here's the humbling bit. My tidy three don't own the top of it. B·B·E does lead, as a weekly setup. But A·A·E, the best single row in the table above, lands around sixth, and A·A·D, the third pattern I was so pleased with, doesn't even make the combined top ten. Five other shapes outrank A·A·E, some of them mixed C-grade combos I'd written off as middling. The ranking weighs the evidence the way I should have all along: a pattern with twice the trades behind it earns its place over a flashier one with barely a few hundred. The three patterns got me started. The ranking is what I actually read off the screen now.
...
## Strategy timeframe matters too

One more layer before we get to the screen, and it's the piece that pulled the whole ranking into focus.

Everything above is about the shape across the three maturity grades, the daily, weekly and monthly stacked into a pattern like B·B·E. That part holds. But a shape on its own doesn't tell you which of my strategies should be trading it. My system runs on more than one clock: a daily strategy and a weekly strategy, each taking its own entries. (Don't mix this up with the maturity grades. One is how I read the stock's move. The other is which strategy actually pulls the trigger.)

So the ranking doesn't grade daily and weekly in their own separate boxes. It cross-ranks them, both timeframes thrown into one list and scored on the same criteria: how rarely a combo hits its stop, what it pays when it works, weighted by how many trades stand behind it. Those two are also the only ones that count here, because the monthly strategy caps out lower down the grades and never carries my top TSLC-A and TSLC-B trades.

And that's where it gets interesting. The same shape can land in two completely different places depending on which strategy is holding it. Take B·B·E. As a weekly setup it sits at #1, the single best thing on the board. As a daily setup, the exact same B·B·E shape drops to #13. Same three letters, wildly different outcome, purely because of the strategy timeframe underneath it.

So there's a ripeness to the strategy timeframe itself, not just to the stock. The shape tells you where the stock is in its cycle. The cross-ranking tells you which of my strategies is ripe to trade that shape. Get the shape right but put it on the wrong clock, and you've taken a #1 setup and turned it into a #13 one. You need both, read together. That's the slice that gave me the final ranking sequence I trade off now.
...
Quick note on the links below: they all open in a new tab. If you would rather grab your own copy to keep, just right-click the link and choose Save link as.
