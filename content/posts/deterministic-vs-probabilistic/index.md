...
And no, setting `temperature = 0` does not save you. Temperature is just the knob for how wild those dice rolls get. Turn it down and the model plays safe, leaning hard on its top pick. Turn it up and it gets creative (a polite word for it makes more up). People swear that dialling it to zero makes the thing predictable. It doesn't... the odds get sharper, the dice are still dice. A raw LLM cannot be made 100% deterministic. That is simply how the thing works.
