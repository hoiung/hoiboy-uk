...
| C — 1h reminder | 1 hour before event starts | Send email to attendee | ✅ Active — **complementary to Cal.com's hardcoded 1h calendar reminder** (see Lessons #11). Different sender + format; safety net for attendees whose calendar app strips embedded reminders. |
...
11. **Cal.com REPLACES Google Calendar default reminders with hardcoded 1-hour reminders.** When Cal.com creates events on the host's connected Google Calendar, it sets `reminders: {useDefault: false, overrides: [{method: "email", minutes: 60}, {method: "popup", minutes: 60}]}`. Your 30-min default doesn't apply; Cal.com's 1-hour reminder fires instead. (Originally captured this as "Cal.com strips all reminders" — that was wrong. Surfaced 2026-05-08 when host saw `Notifications only apply to you` looking at the WRONG Google account's calendar; on the CORRECT account that Cal.com is connected to, the 1-hour reminders are present, just overriding the configured default.) **Implications**:
    - Host: gets 1h email + 1h popup reminders automatically (good), but cannot change to the configured 30-min default without manual per-event editing or a Cal.com setting override.
    - Attendees: receive the iCal invite with 1h reminders embedded. Whether their calendar app honors embedded reminders varies (Google Calendar honors; some Outlook configs may not).
    - **Brevo Template C (1h attendee reminder) is complementary, not redundant**: even though it fires at the same T-1h as Cal.com's email reminder, the sender (`hello@hoiboy.uk` vs Cal.com) and format (Hoi-voice prose vs Cal.com calendar UI) differ. Doubles as safety net for any attendee whose calendar app strips embedded reminders.
    - **Brevo Template 6 (Operator at T-2h)** is the FIRST timing nudge for the host, since Cal.com's hardcoded reminders don't fire until T-1h.
    - **TODO**: investigate Cal.com Settings → Calendars → Google Calendar integration for a "Default event reminders" config that lets us replace the hardcoded 1h with a custom value (e.g. 30 min or none). Low priority because current 1h aligns naturally with our T-1h Brevo Template C.
