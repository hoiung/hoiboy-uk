...
Three of those four controls (browser user-agent, empty user-agent, `Googlebot`) are implemented in `scripts/check-ai-crawler-access.sh` and print under a `CONTROLS` heading on every run, so the standing gate reproduces its own evidence rather than citing a measurement nobody can re-run. They are reported only and do not touch the exit code. **Corrected 2026-07-21:** this sentence used to end "which the citation class alone gates", which was true when written and was falsified by blog-priv#55 Stage 5. BOTH classes now gate: a citation-class crawler blocked, or a training-class token with no full opt-out, each exits 1 and they need opposite fixes. The controls remain report-only. The fourth control, ruling out rate limiting by re-probing in isolation with three-second gaps and by repeating runs, was a manual one-off during Phase 10; the 403s were deterministic across those runs. It is not in the script, because a gate that sleeps between every probe is too slow to run weekly.
...
The two readings cover different bot sets and the counts are NOT directly comparable. The Stage 3 probe covered five training tokens; `Bytespider` was in the class table above but was never probed, so the table under-covered its own taxonomy. That gap was found in Ralph round 10 and closed here: `Bytespider` was added to `TRAINING_BOTS`, probed twice with three-second spacing (403 both times, deterministic), and added to the table. **Superseded 2026-07-21.** At the time of writing the training class was six tokens in the taxonomy, six in the script and six in the measured table, against the **nine** tokens Cloudflare's managed robots.txt disallows, so the probe re-checked six of those nine. That gap was closed in commit `a587230`: `Amazonbot`, `Applebot-Extended` and `CloudflareBrowserRenderingCrawler` were added, the probe now covers **all nine**, and a test pins the list against the managed block so the two cannot drift apart again. The reason it mattered is that the training class had by then been given authority to fail CI, and a gate that never asks about a token cannot fail on it. Both numbers are correct for different sets; see `docs/research/17_AI_CRAWLER_FRAMEWORK.md`.
...
So the preset does real work today, on a subset of crawlers. **Updated 2026-07-21:** this sentence used to read "Until then robots.txt carries the rest", which had no antecedent (no date is named in this section) and is now also out of date. A WAF custom rule was applied to all three zones on 2026-07-21 and hard-blocks seven of the nine tokens at the edge, including `GPTBot`, `ClaudeBot` and `meta-externalagent`, which the preset does not reach. robots.txt carries only the two control tokens that send no user-agent of their own, and for those it remains honour-system because no edge rule can match them. Live position: `17_AI_CRAWLER_FRAMEWORK.md`.

**It is NOT set-and-forget.** This paragraph previously said the preset was set on all three zones so the migration would activate "natively without a revisit". That is retracted: on 2026-09-15 the same setting also causes Googlebot, Applebot and BingBot to be blocked, because Cloudflare enforces the defaults by the most restrictive applicable rule and these zones have selected to block Training. A decision is REQUIRED before that date. `scripts/check_ai_training_deadline.py` turns CI red from 2026-09-01 until one is recorded. RESOLVED 2026-07-22: opted out (mixed-purpose search+training crawlers kept, so Google/Bing/Apple survive the migration); the doc 17 marker records the decision and the gate is green. Full detail, sources and the recommendation: `docs/research/17_AI_CRAWLER_FRAMEWORK.md` "MANDATORY REVISIT BEFORE 2026-09-15".

### Standing gate

`scripts/check-ai-crawler-access.sh` exits non-zero on EITHER a blocked citation-class crawler or a training-class token with no full opt-out (the training half was added by blog-priv#55 Stage 5; this line previously named only the citation half). A CONFLICT verdict is reported and deliberately not failed. `.github/workflows/ai-crawler-access.yml` runs it weekly and on demand, non-blocking, because an edge setting can regress with no repo change and nothing in the repo would show it.
