...
- **Recurrence ceiling.** Reuse `cones.svg` in at most 4 posts and `harness-layers.svg` in at most 3. Only the two named core explainers (`sst3-ai-harness-reshapeable-knife` and `every-sme-needs-their-own-harness`) carry BOTH diagrams; every other post carries at most one.
